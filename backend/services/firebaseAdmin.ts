import { getAdminDb } from "@/backend/firebaseAdmin";
...
export class FirestoreAdminService {
  ...
  static async read(collectionName: string, docId: string): Promise<any | null> {
    ...
    if (docSnap.exists) {
      return { id: docId, ...docSnap.data() } as any;
    }
    return null;
  }
  ...
  static async getWhere(collectionName: string, field: string, operator: FirebaseFirestore.WhereFilterOp, value: any): Promise<any[]> {
    const db = getAdminDb();
...
export class ChurchDataAdminService {
  ...
  static async getStatsByType(type: string, since?: Date): Promise<any[]> {
    ...
    if (!since) return all;
    return all.filter((e) => {
      const d = (e?.createdAt as unknown) as Date | { seconds?: number } | undefined;
      if (!d) return false;
